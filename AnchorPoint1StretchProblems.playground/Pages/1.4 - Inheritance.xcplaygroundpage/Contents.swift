...
/*:
 ...
 5. Give the `Bird` subclass a function named `fly()` which prints "WHOOSH... <insert the birds name> is Flying" (Notice the bird subclass still has access to the `name` variable we declared to be part of animal)
 6. Subclass `Bird` to create a `Penguin` subclass.
 7. Give the `Penguin` class a function called 'feedYoung()' which prints "<insert the penguins name> is regurgitating some <insert the penguins diet> to it's young"
 8.Initialize a penguin and try calling all of its available functions.
 ...
 9. Override the `fly()` property for penguins and substitute an implementation which prints that penguins can't fly.
 
 10. Create a `Dog` subclass of `Animal` with a function called `bark()` that prints "WOOF WOOF... <insert dogs name> is barking"
 
 ### Black Diamonds 💎💎💎
 
 1. Add functionality to the `eat()` function which the `Dog` class inherits to print "<insert dogs name> made a mess"
 Note: This should not replace the `Animal` class's implementation of `eat()` only add onto it.
 2. Create a `Poodle` class which inherits from (is a subclass of) `Dog`.  Give the `Poodle` class a `rankInStateFair` variable with NO default value.  Write an initalizer for the `Poodle` class
